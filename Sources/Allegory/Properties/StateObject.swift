...
public struct StateObject<ObjectType>: DynamicProperty
    ...
    public var wrappedValue: ObjectType {
        guard case let .object(object) = storage else { error() }
        return object.wrappedValue
    }

    public var projectedValue: RxObservedObject<ObjectType>.Wrapper {
        guard case let .object(object) = storage else { error() }
        return object.projectedValue
    }

    mutating func setContent(_ value: Any) {
        guard case let .initially(initial) = storage else { return }
        storage = .object(.init(wrappedValue: value as? ObjectType ?? initial()))
    }
